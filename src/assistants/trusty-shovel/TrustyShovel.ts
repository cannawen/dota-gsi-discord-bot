...
export default class TrustyShovel {
    ...
    public items(items: PlayerItems): void {
        if (items.neutral?.id === "item_trusty_shovel") {
            this.shovelCanBeUsed = items.neutral.canCast
                ? items.neutral.canCast
                : false;
        }
    }
}
...
broker.register(
    "ASSISTANT/TRUSTY_SHOVEL/ITEMS",
    Topic.DOTA_2_ITEMS,
    null,
    component.items.bind(component)
);
broker.register(
    "ASSISTANT/TRUSTY_SHOVEL/TIME",
    Topic.DOTA_2_TIME,
